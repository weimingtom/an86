...
package com.google.protobuf
{
	...
	public class Message {
	  ...
	  public function writeToCodedStream(output:CodedOutputStream ):void {
        ...
        for each (var desc:Descriptor in fieldDescriptors) 
        {
			...
			{	
				...
				if (desc.isRepeated() && (this[desc.fieldName] is Array || String(this[desc.fieldName]).indexOf('Vector')))
				{
					for each( var elem:* in this[desc.fieldName])
					{
						...
						//}
						//else //primative
						//	output.writeField(desc.type, desc.fieldNumber, elem);
						output.writeField(desc.type, desc.fieldNumber, elem);
					}
				}
				else 
				{
					...
					//}
					//else //primative
					//	output.writeField(desc.type, desc.fieldNumber, this[desc.fieldName]);
					output.writeField(desc.type, desc.fieldNumber, this[desc.fieldName]);
				}
			}
        }
	  ...
	  */
	  public function getSerializedSize():int {
	    ...
	    for each (var desc:Descriptor in fieldDescriptors) 
	    {
	    	...
	    	if (this[desc.fieldName] != null)
				size += CodedOutputStream.computeFieldSize(desc.type, desc.fieldNumber, this[desc.fieldName]);
		} 
	  ...
	  */
	  ...
	  public function toDump(deep:int=0):String{
		  var str:String = "";
		  var dstr:String = "";
		  for(var i:int=0;i<deep;i++)dstr+="    ";
		  for(var field:String in fieldDescriptors){
			  if(this[field] is ByteArray) str += dstr + "[" + field + "] = ByteArray\n";
			  else if(this[field] is Message){ str += dstr + "[" + field + "] = Message\n"; str += this[field].toDump(deep+1);}
			  else str += dstr + "[" + field + "] = " + this[field] + "\n";
			  
			  if(this[field] !=null && this[field].hasOwnProperty("length") && !(this[field] is String) && !(this[field] is ByteArray)){
				  for(var j:int=0;j<this[field].length;j++){
					  if(this[field] is ByteArray) str += dstr + "    " + "[" + j + "] = " + this[field][j] + "\n";
					  else if(this[field][j] is Message) str += this[field][j].toDump(deep+2);
					  else str += dstr + "    " + "[" + j + "] = " + this[field] + "\n";
				  }
			  }
		  }
		  return str;
