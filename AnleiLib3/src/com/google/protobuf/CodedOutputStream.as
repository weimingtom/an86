...
{
	import anlei.util.CustomerByteArray;
	...
	public final class CodedOutputStream {
	  ...
	  public function writeDoubleBytes(fieldNumber:int, value:Number):void {
	 ...
	 /**
	   ...
	   *               this field.
	   */
	  public function writeField(type:int, number:int, value:*):void {
		  
		  switch (type) {
			  //case Descriptor.DOUBLE  : writeDouble(number, new BigInteger(value as Number));break;;
			  //case Descriptor.FLOAT   : writeFloat(number, value as Number);
			  case Descriptor.INT64   : writeInt64(number, new BigInteger(value as Number));break;
			  case Descriptor.UINT64  : writeUInt64(number, new BigInteger(value as Number));break;
			  case Descriptor.INT32   : writeInt32(number, value as int);break;
			  //case Descriptor.FIXED64 : writeFixed64(number, new BigInteger(value as Number));break;
			  //case Descriptor.FIXED32 : writeFixed32(number, value as int);break;
			  case Descriptor.BOOL    : writeBool(number, value as Boolean);break;
			  case Descriptor.STRING  : writeString(number, value as String);break;
			  case Descriptor.GROUP   : writeMessage (number, value as Message);break;
			  case Descriptor.MESSAGE : writeMessage(number, value as Message);break;
			  case Descriptor.BYTES   : writeBytes(number, value as ByteArray);break;
			  case Descriptor.UINT32  : writeUInt32(number,value as uint);break;
			  //case Descriptor.ENUM    : writeEnum(number, value as int);break;
			  //case Descriptor.SFIXED32: writeSFixed32(number, value as int);break;
			  //case Descriptor.SFIXED64: writeUInt64(number, new BigInteger(value as Number));break;
			  //case Descriptor.SINT32  : writeSInt32(number, value as int);break;
			  //case Descriptor.SINT64  : writeSInt64(number, new BigInteger(value as Number));break;
			  default: 
	    ...
	    /*if (value is String)
			...
			if(value.toString() == 'NaN') value = 0;
			writeDoubleBytes(number, Number(value));
		}
	    else
	    	throw  new InvalidProtocolBufferException( "Tried to write primative field type, but type was not valid");*/ 
	  ...
	  public static function computeFieldSize(type:int, number:int, value:*):int {
		  
		  switch (type) {
			  //case Descriptor.DOUBLE  : return computeDoubleSize(number, value as Number);
			  //case Descriptor.FLOAT   : return computeFloatSize(number, value as Number);
			  case Descriptor.INT64   : return computeInt64Size(number, new BigInteger(value as Number));//bigInt << number
			  case Descriptor.UINT64  : return computeUInt64Size(number, new BigInteger(value as Number));//bigInt << number
			  case Descriptor.INT32   : return computeInt32Size(number, value as int);
			  //case Descriptor.FIXED64 : return computeFixed64Size(number, value as Number);//number
			  //case Descriptor.FIXED32 : return computeFixed32Size(number, value as int);
			  case Descriptor.BOOL    : return computeBoolSize(number, value as Boolean);
			  case Descriptor.STRING  : return computeStringSize(number, value as String);
			  //case Descriptor.GROUP   : return computeGroupSize(number, value as Message);//MESSAGE
			  //case Descriptor.MESSAGE : return computeMessageSize(number, value as Message);
			  case Descriptor.BYTES   : return computeBytesSize(number, value as ByteArray);
			  case Descriptor.UINT32  : return computeUInt32Size(number, value as int);
			  //case Descriptor.ENUM    : return computeEnumSize(number, value as int);
			  //case Descriptor.SFIXED32: return computeSFixed32Size(number, value as int);
			  //case Descriptor.SFIXED64: return computeFixed64Size(number, value as Number);//number
			  //case Descriptor.SINT32  : return computeSInt32Size(number, value as int);
			  //case Descriptor.SINT64  : return computeSFixed64Size(number, new BigInteger(value as Number));//bigInt << number
			  default: 
	    ...
	    /*if (value is String)
			...
			return computeFixed64Size(number, Number(value));
	    else
	    	throw  new InvalidProtocolBufferException( "Could not compute size of field, type was not valid");*/
