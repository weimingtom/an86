package ui.component
{
	...
	public class MyAccordion extends MySprite implements AbstractComponent
	{
		...
		private function inits():void{
			...
			this.addEventListener(MouseEvent.DOUBLE_CLICK, function(e):void{
				removeAllTab();
			});*/
		}
		...
		private function resetCoor():void
		{
			...
			for(var j:int = 0 ; j < data_arr.length; j++){
				allh += data_arr[i].titleHeight;
			}
			
			for(var i:int = 0 ; i < data_arr.length; i++)
			{
				data_arr[i].y = temph;
				temph += data_arr[i].titleHeight;
				
				data_arr[i].x = 0;
				data_arr[i].ID = i;
				//data_arr[i].setChildSize(INIT_Width, INIT_Height - allh);
				data_arr[i].setTitleWidth(INIT_Width);
				//data_arr[i].update();
			}
		}
		...
		public function openItem(id:uint):void{
			...
			if(it.isDown){
				for(var j:uint = 0 ; j < index + 1; j++){
					...
					it.isDown = false;
					TweenLite.to(it, 0.3, {y:it.titleHeight * it.ID, onComplete:onComplete});
				...
				for(var i:uint = index + 1; i < len; i++){
					...
					it.isDown = true;
					TweenLite.to(it, 0.3, {y:INIT_Height-(it.titleHeight*(len-i)), onComplete:onComplete});
